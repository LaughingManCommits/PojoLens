...
| CSV onboarding | `Compatibility adapter` | Boundary-only loading from UTF-8 CSV into typed rows, with optional runtime-owned defaults and explicit coercion policy | `PojoLensCsv`, `CsvOptions`, `CsvCoercionPolicy`, `CsvRuntime`, `PojoLensRuntime` | `Advanced` adapter surface | `README.md`, `docs/entry-points.md`, `docs/csv.md` |
...
- `PojoLensCsv` is a boundary-only onboarding helper. It loads typed rows into
  ...
  variation handling explicit instead of inferred.
