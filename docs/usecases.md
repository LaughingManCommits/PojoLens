...
| Code-owned typed filters, joins, grouped aggregates, and ordering | `TypedQuery.from(rowType)` | Stable Java-owned foundation for generated `TypedField<T,V>` constants, projection, filters, join declarations with `JoinBindings` / `DatasetBundle`, grouped aggregates, ordering, offset, limit, explain/schema, and execution guards. |
