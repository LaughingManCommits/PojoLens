...
| Code-owned typed filters, joins, grouped aggregates, `HAVING`, windows, and ordering | `TypedQuery.from(rowType)` | Stable Java-owned foundation for generated `TypedField<T,V>` constants, projection, filters, join declarations with `JoinBindings` / `DatasetBundle`, grouped aggregates, grouped `HAVING` over grouped fields and metric aliases, rank/running window outputs with `QUALIFY`, ordering, offset, limit, explain/schema, and execution guards. |
