...
| Code-owned typed filters, joins, grouped aggregates, `HAVING`, windows, and ordering | `TypedQuery.from(rowType)` | Stable Java-owned foundation for generated `TypedField<T,V>` constants, projection, filters, join declarations with `JoinBindings` / `DatasetBundle`, grouped aggregates, grouped `HAVING` over grouped fields and metric aliases, rank windows, aggregate window outputs with `QUALIFY`, explicit aggregate window frames via `QueryWindowFrame`, ordering, offset, limit, explain/schema, and execution guards. |
