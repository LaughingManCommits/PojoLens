...
- `2026-04-29`: WP24 is in progress. `TypedQuery.having(...)` now supports grouped-output predicates over grouped fields and metric aliases without adding a second grouped-query surface; typed windows and bounded subqueries remain open.
...
- `2026-04-29`: WP23 and the first WP24 slice both passed focused typed/public-API coverage, full reactor `mvn -B -ntp test`, `mvn -B -ntp -Pstatic-analysis verify -DskipTests`, and `scripts/check-doc-consistency.ps1`.
...
- `2026-04-29`: Continue WP24 with typed windows and bounded typed subquery/existence design after the `having(...)` slice.
