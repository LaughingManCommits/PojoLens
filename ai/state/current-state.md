...
- `2026-03-31`: simple direct SQL-like scatter charts now bypass `QueryRow` materialization when the query is a plain source-backed chart; warmed scatter GC reruns put direct and bound at near-allocation parity while leaving a smaller shared SQL-like-vs-fluent gap.
...
- Direct and bound SQL-like scatter now benchmark at near-parity, so the remaining scatter gap is shared SQL-like execution/projection overhead rather than repeated direct-path source rebind/materialization.
...
- If benchmark follow-up resumes, use `TODO.md` plus the `2026-03-31` artifacts, then focus on the residual shared SQL-like scatter allocation gap and the broader hotspots.
