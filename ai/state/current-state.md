...
- `2026-04-29`: WP24 is in progress. `TypedQuery` now covers grouped `HAVING`, rank windows, aggregate window frames via `QueryWindowFrame`, and `QUALIFY` on the same immutable surface; typed subqueries remain open.
...
- `2026-04-29`: WP23 plus the WP24 `HAVING`, window/`QUALIFY`, and bounded-frame slices passed focused typed/public-API coverage, full reactor `mvn -B -ntp test`, `mvn -B -ntp -Pstatic-analysis verify -DskipTests`, and `scripts/check-doc-consistency.ps1`.
...
- `2026-04-29`: Continue WP24 with typed subquery/existence composition now that aggregate window frames are on `TypedQuery`.
