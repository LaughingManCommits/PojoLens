...
- AI orchestration spike is reopened; `WP9` is complete and `WP10` dependency-state materialization is next.
...
- For the AI orchestration spike, start with `WP10` dependency-state materialization, then a stronger live proof in `WP13`, then resume/inventory/retention in `WP11`.
