...
3. Follow `TODO.md`: WP45 -> WP46 -> WP40 -> deferred WP18 -> Release Gate.
4. Treat `Release Gate` as last and cut from `RELEASE.md` only when requested.

## Focus
- `2026-05-02`: WP44 is complete. `asyncio.Semaphore` + `asyncio.as_completed` replace `ThreadPoolExecutor`; `execute_task`/`execute_task_with_retry`/`run_loaded_plan` are `async def`; `asyncio.create_subprocess_exec` in `provider.py`; `run_subprocess_async` in `provider_worker.py`; SDK via `asyncio.to_thread`; sync CLI entry via `asyncio.run()`. 484 green.
...
- `2026-05-02`: Next queue is WP45 (OpenTelemetry) through WP46, then WP40, then deferred WP18 and Release Gate.

## Facts
- `2026-05-02`: `run_ops.run_loaded_plan` is `async def`; called from sync `orchestrator_app.run_loaded_plan` via `asyncio.run()`; test entry points and CLI shim unchanged.
- `2026-05-02`: `run_subprocess_async` in `provider_worker.py` is the async subprocess path; sync `run_subprocess` kept for planner path.
...
- `2026-05-02`: `TODO.md` order is WP45, WP46, WP40, deferred WP18, Release Gate (WP44 complete).
