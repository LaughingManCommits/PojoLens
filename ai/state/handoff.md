...
- Claude orchestration uses repo-local `.claude-orchestrator/`; prompt sections/budgets, sparse-copy workspaces, overlapping-scope serialization, protected-path audits, `review`, and `export-patch` are now in place, and the next spike slice is apply/promote ergonomics.
