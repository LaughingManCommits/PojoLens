...
- March 31 chart reruns under `target/benchmarks/2026-03-31-followup-stability/` stabilized threshold and parity.
- Plain SQL-like scatter charts now skip `QueryRow` materialization; direct and bound stay near-allocation parity.
...
- Root `SPIKE-CSV.md` argues CSV fits only as a bounded adapter into the existing engine.
...
- Before live runs, inspect the dry-run manifest for prompt size and model choice; use `modelProfile = simple|balanced|complex` unless `model` must be explicit.
