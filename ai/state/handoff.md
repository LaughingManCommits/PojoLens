...
- `2026-04-29`: WP24 is in progress. `TypedQuery` now covers grouped `HAVING`, rank windows, aggregate window frames via `QueryWindowFrame`, and `QUALIFY` on the same immutable typed surface; typed subqueries are still pending design.
...
- `2026-04-29`: WP24 outcome so far: typed `having(...)` accepts grouped-output predicates only, typed windows reuse `window(...)` / `windowCountAll(...)` plus `TypedWindowOrder`, aggregate frames reuse `QueryWindowFrame`, `qualify(...)` stays scoped to selected window aliases, and only typed subqueries remain text-surface-only for now.
