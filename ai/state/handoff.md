...
- A quick targeted 2026-03-14 JMH verification at `size=10000` measured `StatsQueryJmhBenchmark.sqlLikeParseAndTimeBucketMetricsToChart` at about `7.088 ms/op` versus `7.200 ms/op` for `fluentTimeBucketMetricsToChart` under the short verification run.
...
- The next WP18 opportunities after the prepared execution change are likely chart payload assembly, projection materialization, or any remaining per-row reflection/conversion cost that still dominates after SQL-like bind/setup is cheaper.
...
- Should the next WP18 pass cache or pre-plan chart payload mapping itself, or is the remaining cost now mostly below SQL-like setup in projection/chart assembly?
