...
- A new uncommitted 2026-03-19 join follow-up now adds dense integer-key child indexing in `FastArrayQuerySupport.buildChildIndex(...)` when child join keys are non-negative `Integer` values in a reasonable range, with hash-index fallback for non-dense keys.
- Benchmark guardrail CI was fixed on 2026-03-19 by adding threshold entries for `PojoLensJoinJmhBenchmark.pojoLensJoinLeftComputedFieldOrderedLimited|size=1000/10000`; then the whole core threshold file was rebaselined from CI report scores with `ceil(score * 1.5, 0.1ms)` so strict guardrails are tighter and less stale.
...
- A short-lived primitive sidecar spike for numeric computed fields regressed both latency and allocation and was reverted in-session.
- A follow-up dense child-index pass now has targeted benchmark artifacts at `target/wip-bench-dense-index-2026-03-19.json` and `target/wip-bench-dense-index-gc-2026-03-19.json`; relative to the immediate pre-change baseline (`target/wip-bench-sidecar-off-2026-03-19.json` / `target/wip-bench-sidecar-off-gc-2026-03-19.json`), it showed modest latency wins on most tracked shapes plus lower `size=10000` allocation/op.
...
- `benchmarks/thresholds.json` no longer uses the older loose baseline set; on 2026-03-19 it was rebaselined to CI-observed scores with 1.5x headroom per key, and strict local verification still passes.
...
- Do not reopen WP20 unless the computed-field join implementation changes again enough to invalidate the current CI-rebased cold strict budgets in `benchmarks/thresholds.json`.
...
- For the new dense child-index path, keep the dense-key guard conservative (`non-negative Integer` keys and bounded range) and retain hash fallback unless repeated benchmarks on your datasets show a wider safe envelope.
- Current core thresholds are intentionally derived from CI-observed scores with a 1.5x cushion; if you tune them again, use repeated CI strict reruns and rebase systematically instead of per-key ad hoc edits.
