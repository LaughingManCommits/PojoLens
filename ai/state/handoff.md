...
- Natural queries now support grouped aggregates, time buckets/chart phrases, explicit `from ... join ... on ...`, deterministic window phrases, and alias-based `qualify`.
...
- Contextual natural explain adds resolved field/sql-like metadata plus chart metadata when chart phrases are present; joined explains also carry `joinSourceBindings`.
...
- Next natural-query slice is likely computed-field phrasing and/or natural templates.
...
- Natural window/qualify slice last validated with `mvn -q -pl pojo-lens test`
