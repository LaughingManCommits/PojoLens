...
- dependency handoff should include a stable branch-context identifier so downstream tasks and retained manifests can tell which upstream branch produced the summary or reviewed layer
...
- Agent and task definitions may carry optional `extraTools` arrays; each entry declares `name`, `description`, `kind` (`shell` or `script`), `template`, and optional `timeoutSec`; task-level `extraTools` override agent-level when non-empty; tool names must not collide with the five base workspace tools (`read_file`, `write_file`, `str_replace_based_edit_tool`, `bash`, `write_shared_context`) and templates must not contain `..`; the executor runs the resolved template via a subprocess shell rooted at the task workspace and caps output at the max tool output limit; validate surfaces expose resolved tool names per agent and per task.
- SDK-provider workers have access to a fifth base tool `write_shared_context(note, tags?)`; calling it appends a `{ts, taskId, note, tags}` JSON line to the run-local `shared-context.jsonl` scratchpad; downstream task prompts inject the bounded tail of that file (default 10 lines) under a "Shared context notes" section; task definitions may declare `sharedContextTags: list[str]` to filter injected lines to entries whose `tags` include at least one matching tag; the scratchpad path (`sharedContextPath`) is persisted in the run manifest alongside `runDir`.
...
- Optional run-level governance should be expressible in tracked plans via `runPolicy`; `budgetBehavior` / `artifactBehavior` should support `warn` and `stop`, and `stop` should block unscheduled later batches rather than trying to cancel already-running tasks; a `budgetBehavior = "stop"` cap should emit a `budget-exceeded` run event with `{actualCostUsd, limitCostUsd, remainingTaskIds}`, set `lifecycleState = "budget_exceeded"`, include `budgetExceeded: true` in the run payload, and exit with code `8`.
- Validate should warn when tracked `runPolicy.runBudgetUsd` is already lower than the minimum pre-flight estimate.
- Human-in-the-loop gates should be expressible through tracked `runPolicy` fields and one-off `run` / `resume` CLI overrides. A gate fires after a completed batch before later scheduling, emits `hitl-gate`, persists the manifest, waits for stdin or a run-local `hitl-gate.lock` sentinel decision, then emits `hitl-approved` or `hitl-aborted`; aborted gates should block pending tasks with an explicit coordinator reason. Mode semantics: `always` fires before **every** batch without exception; `batch` fires before every batch with pending tasks; `on-failure` fires only when the completed batch contained at least one failure. The sentinel file must be validated by `gateId` — if the file contains JSON with a `gateId` that does not match the current gate, it must be treated as absent so stale decisions from prior interrupted gates are never acted upon.
...
- Worker prompts should keep structured output bounded: `summary` should stay short, `notes` / `followUps` should stay capped to a few high-signal items, `validationIntents` should stay capped to a few high-signal suggestions, and optional structured `followUpTasks` should stay small enough for coordinator review and validation.
...
- Live worker results may also emit structured `followUpTasks`; these are distinct from human-readable `followUps` and are the only coordinator-supported input for runtime task injection; each entry may carry an optional `conditionField`/`conditionValue` predicate (both must be set or both omitted) — if present, the coordinator performs a case-insensitive substring match of `conditionValue` against the named field of the emitter task's run record before injection; tasks that fail the condition emit `task-injection-skipped` with the field, expected value, and actual value in their event details; tasks with no condition are always injected when `followUpBehavior = inject`.
...
- Same-run resume should reload from the run-local `selected-plan.json` snapshot so any previously injected follow-up tasks remain part of the retained plan graph.
...
- The coordinator should also support optional OTEL emission from that retained span graph when `OTEL_EXPORTER_OTLP_ENDPOINT` or an equivalent CLI override is set, so standard OTLP backends can ingest run, batch, task, validation, and approval spans without a repo-specific converter.
...
- When retained completed tasks touch only docs-like text files and no equivalent docs validation was already suggested, the coordinator should synthesize a repo-scope docs consistency helper so documentation-oriented runs still present a default validation path before promotion.
...
- The rate limiter is advisory for individual tasks: it pre-deducts estimated tokens before dispatch and charges only the positive delta of `(actual − estimated)` as a correction after the task record arrives; actual overruns within a running batch are absorbed, and future batches are throttled based on the corrected window state; the limiter cannot retroactively cancel a task already in flight.
...
- When `followUpBehavior = inject`, the coordinator should validate each structured `followUpTask`, force a dependency edge from the emitting task, reject duplicate or unresolved task ids, re-run task-graph and scope-contract validation, persist accepted injections back into `selected-plan.json`, and emit `task-injected` or rejection events before later scheduling.
- Review and promotion surfaces should apply docs/text guardrails for docs-like text files: block common mojibake or encoding-corruption patterns, warn when new non-ASCII text is introduced into an otherwise ASCII baseline unless the repo already used Unicode in that file, and surface those findings directly in retained review and promotion summaries.
...
- Worker prompts should also require explicit `notes` for parameter/default/normalization semantics, require tests to align with the implemented public contract, and require docs updates or explicit follow-ups when user-visible example/API behavior changes.
...
- Coordinator-side promotion should refuse `workspaceMode="repo"` task changes, protected-path violations, conflicting changed-file ownership across selected tasks, and path traversal outside the repo root; exact duplicate operations with matching workspace content may be deduped automatically.
...
- A coding run with applied promotion should not be considered complete until repo-scope coordinator validation is recorded after promotion and is passing.
