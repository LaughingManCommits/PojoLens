...
- The operator surface may also expose a guided wizard entry that defaults from no CLI subcommand, wraps validate -> run -> review -> promote -> validate-run, supports `--resume` / `--retry` shortcuts for retained runs, and can optionally map a natural-language goal onto a tracked or runtime-generated task plan using the cheapest planner-capable model.
