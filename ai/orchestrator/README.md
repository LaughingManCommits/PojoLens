...
- worker prompts now tell workers to mirror declared validation hints when they already use approved entrypoints, use `repo-script` only for `scripts/...` or `mvnw(.cmd)`, use `tool` only for approved executables, and emit `[]` instead of inventing `grep`, pseudo scripts, or shell fragments
